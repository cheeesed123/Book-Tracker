# Book Tracker
A basic terminal-based tracker for reading books. Uses CSVs.

This is a simple CSV manager, basically. The Java program creates "Book" objects, which are stored in a CSV. This program runs on Windows, Mac, and Linux environments.
If you edit the CSV, it doesn't care. This program was made as a fun project, mostly.
## Features:

 - Make a new book (New book)
 - List books in CSV (List books)
 - Edit the number of read pages in the book (Edit book)
 - Edit all the other information (Other edit)
 - Calculates how long it'll take you to read a book by pages per day. (Stats on time)
 - Reset options (Reset zone), which allow you to refresh and reset data.
 - Power cycle for program (Cycle)
 - Quit functionality

## Tips:

* If you have a lot of books, and there's a text box saying to `Hit enter to escape or type a number to view a book,` you can type a book number to view that book, and even type another book number after that one to view yet another, and so on.
* Typos and incorrect keys are caught automatically.
* If you accidentally enter a menu from the main menu, you can hit enter to exit.
* It has an installer.

If you have any questions, you can reach out to me at **olliessmith2910@gmail.com**.

Book Tracker V3.0
